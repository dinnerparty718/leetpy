...
# Time O(M(4*3 ^ L-1))   M number of Cell, L max lenth of words
# space O(N)  N is the total number of letters in the dict
...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(i: int, j: int, parent):
            letter = board[i][j]
            curNode = parent[letter]

            # check if we find a match of word
            # brilliant
            # also remove the word to avoid duplicate
